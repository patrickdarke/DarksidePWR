...
// ---------------------------------------------------------------------------
// ELECROW CrowPanel Advance 5.0" (800x480) — the second, new install. Same
// interaction model as the 3.5" build (OFF needs a second confirming tap,
// writes queue via gxWrite() and highlight GX read-back), just laid out with
// the extra room this canvas gives. The install has two SmartSolar chargers,
// so SOLAR becomes one labeled ON/OFF row per charger (GX_SOLAR_LABELS)
// instead of one fixed "SOLAR" row.
// ---------------------------------------------------------------------------
namespace {
...
constexpr int kSolarRows = GxData::kNumSolar ? GxData::kNumSolar : 1;  // pad, see GxData
lv_obj_t* s_solarBtn[kSolarRows][2] = {{nullptr}};  // [charger][ON/OFF]
lv_obj_t* s_altNa = nullptr;
lv_obj_t* s_hint = nullptr;
lv_timer_t* s_offTimer = nullptr;        // disarms the OFF confirm

const char* kSolarNames[] = GX_SOLAR_LABELS;
static_assert(sizeof(kSolarNames) / sizeof(kSolarNames[0]) == GxData::kNumSolar,
              "GX_SOLAR_LABELS length must match GX_SOLAR_UNITS");
...
// Row geometry: 5 fixed rows + one per solar charger, 54px pitch, 40px
// chips (bigger touch targets than the 3.5" build's 36px — this canvas has
// the room). Assumes the roster fits comfortably below the header; revisit
// if it ever grows past ~3 solar chargers.
constexpr int kRowPitch = 54;
constexpr int kFirstY = 56;
constexpr int kChipH = 40;
...
// user_data packs charger index and ON/OFF into one int: (i << 1) | on.
void solarCb(lv_event_t* e) {
  const int packed = (int)(uintptr_t)lv_event_get_user_data(e);
  const int i = packed >> 1;
  const int on = packed & 1;
  if (i >= GxData::kNumSolar || !s_last.solarModeOk[i]) return;
  sendCmd(kGxSolarUnits[i], 774, on ? 1 : 4);
}
...
void uiCtlBuild() {
  ...
  lv_obj_t* close = mkChip(636, 12, 140, 36, LV_SYMBOL_CLOSE "  CLOSE", closeCb, 0);
  lv_obj_set_style_bg_color(close, lv_color_hex(kRing), 0);

  int y = kFirstY;
  mkRowLabel(y + 14, "MULTI");
  for (int i = 0; i < 4; i++)
    s_mpBtn[i] = mkChip(220 + i * 124, y, 110, kChipH, kMpLabels[i], mpCb, i);

  y += kRowPitch;
  mkRowLabel(y + 14, "SHORE A");
  mkChip(220, y, 80, kChipH, LV_SYMBOL_MINUS, shoreCb, -1);
  s_shoreVal = mkValLabel(310, y + 10, 160);
  mkChip(480, y, 80, kChipH, LV_SYMBOL_PLUS, shoreCb, +1);

  y += kRowPitch;
  mkRowLabel(y + 14, "RELAYS");
  s_relayBtn[0] = mkChip(220, y, 270, kChipH, "R1  --", relayCb, 0);
  s_relayBtn[1] = mkChip(500, y, 270, kChipH, "R2  --", relayCb, 1);

  y += kRowPitch;
  mkRowLabel(y + 14, "CHG LIMIT");
  mkChip(220, y, 80, kChipH, LV_SYMBOL_MINUS, dvccCb, -1);
  s_dvccVal = mkValLabel(310, y + 8, 180);
  mkChip(500, y, 80, kChipH, LV_SYMBOL_PLUS, dvccCb, +1);

  y += kRowPitch;
  mkRowLabel(y + 14, "ALTERNATOR");
  s_altBtn[0] = mkChip(220, y, 150, kChipH, "ON", altCb, 1);
  s_altBtn[1] = mkChip(380, y, 150, kChipH, "OFF", altCb, 0);
  ...
  lv_obj_set_width(s_altNa, 240);
  lv_obj_set_pos(s_altNa, 550, y + 12);
  lv_obj_add_flag(s_altNa, LV_OBJ_FLAG_HIDDEN);

  for (int i = 0; i < GxData::kNumSolar; i++) {
    y += kRowPitch;
    mkRowLabel(y + 14, kSolarNames[i]);
    s_solarBtn[i][0] = mkChip(220, y, 150, kChipH, "ON", solarCb, (i << 1) | 1);
    s_solarBtn[i][1] = mkChip(380, y, 150, kChipH, "OFF", solarCb, (i << 1) | 0);
  }
  ...
  lv_obj_set_pos(s_hint, 24, y + kRowPitch);
...
void uiCtlOpen() {
  // Never self-reference (see ui_setup.cpp's uiSetupOpen() for why).
  lv_obj_t* cur = lv_scr_act();
  if (cur != s_scr) s_prevScr = cur;
  disarmOff();
  setHint(kDefaultHint);
  lv_scr_load(s_scr);
}
...
void uiCtlUpdate(const GxData& d) {
  ...
  for (int i = 0; i < GxData::kNumSolar; i++) {
    if (d.solarModeOk[i]) {
      styleChip(s_solarBtn[i][0], d.solarMode[i] == 1, true);
      styleChip(s_solarBtn[i][1], d.solarMode[i] == 4, true);
    } else {
      styleChip(s_solarBtn[i][0], false, false);
      styleChip(s_solarBtn[i][1], false, false);
    }
  }
...
void solarCb(lv_event_t* e) {
  if (GxData::kNumSolar == 0 || !s_last.solarModeOk[0]) return;
  const int on = (int)(uintptr_t)lv_event_get_user_data(e);
  sendCmd(kGxSolarUnits[0], 774, on ? 1 : 4);
}
...
void uiCtlUpdate(const GxData& d) {
  ...
  if (GxData::kNumSolar > 0 && d.solarModeOk[0]) {
    styleChip(s_solarBtn[0], d.solarMode[0] == 1, true);
    styleChip(s_solarBtn[1], d.solarMode[0] == 4, true);
  } else {
